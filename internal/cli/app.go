...
func (a *App) pickMulti(prompt string, items []string) []string {
	if len(items) == 0 {
		return nil
	}
	if a.Yes {
	...
	for i, it := range items {
		fmt.Fprintf(a.Stderr, "  %s%d%s) %s\n", a.ui.bold, i+1, a.ui.reset, it)
	}
	fmt.Fprintf(a.Stderr, "  numbers (space-separated), or %sall%s: ", a.ui.bold, a.ui.reset)
	line, _ := a.readLine()
	line = strings.TrimSpace(line)
	if line == "" || line == "all" || line == "a" {
		return items
	}
	var out []string
...
func (a *App) pickInstall(prompt string, cands []installCand) []string {
	...
	for i, c := range cands {
		...
		tag := "override  " + c.occ
		if c.kind == "new" {
			mark = a.ui.green + "[x]" + a.ui.reset
			tag = "new"
		}
		fmt.Fprintf(a.Stderr, "  %s%d%s) %s %s  %s%s%s\n", a.ui.bold, i+1, a.ui.reset, mark, c.name, a.ui.dim, tag, a.ui.reset)
	}
	fmt.Fprintf(a.Stderr, "  numbers to install (space-separated), %sall%s, or enter for defaults: ", a.ui.bold, a.ui.reset)
	line, _ := a.readLine()
	line = strings.TrimSpace(line)
	if line == "" {
		var out []string
	...
	for _, tok := range strings.Fields(line) {
		var n int
		if _, err := fmt.Sscanf(tok, "%d", &n); err != nil || n < 1 || n > len(cands) {
			continue
		}
		out = append(out, cands[n-1].name)
	}
	return out
}
